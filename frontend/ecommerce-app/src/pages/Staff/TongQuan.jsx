...
export default function TongQuan() {
    ...
    const fetchExtraData = async () => {
      try {
        const [resTopSp, resHieuSuatCN, chiNhanh] = await Promise.all([
          api.get('/staff/tong-quan/top-san-pham', { 
            params: dashboardParams, 
            headers:{
                Authorization: `Bearer ${token}`
            }}),
          api.get('/staff/tong-quan/hieu-suat-chi-nhanh', { 
            params: dashboardParams,
            headers:{
                Authorization: `Bearer ${token}`
            }}),
          api.get(`/staff/chi-nhanh/`,{
            headers:{
                Authorization: `Bearer ${token}`
            }
          })
  ...
  const layDuLieuDashboard = async () => {
    setDangTai(true);
    try {
      const phanHoi = await api.get("/staff/tong-quan", {
        params: dashboardParams,
        headers: {
            Authorization: `Bearer ${token}`,
        },
  ...
  const layDuLieuBieuDo = async () => {
    ...
    try {
      const res = await api.get(
        "/staff/tong-quan/bieu-do-so-sanh",
        {
          params: dashboardParams,
